...
void vApplicationDaemonTaskStartupHook(void)
{
    ...
    xTaskCreate(thread_adaptor, "adaptor", 160, NULL, osPriorityNormal, NULL);
    ...
    xTaskCreate((TaskFunction_t)Thread_BatteryInfoUpdate, "batinfo1", 192, &Battery_1, osPriorityNormal, NULL);
    xTaskCreate((TaskFunction_t)Thread_BatteryInfoUpdate, "batinfo2", 192, &Battery_2, osPriorityNormal, NULL);
    ...
    xTaskCreate(DebugTask, "debubprint", 160, NULL, osPriorityNormal, NULL);
    xTaskCreate(BatteryDataLog, "batlog", 192, NULL, osPriorityNormal, NULL);
    xTaskCreate(Thread_CANComm, "CANbus", 192, NULL, osPriorityAboveNormal, NULL);
    ValveDataLoad();
    xprintf("os start...\n\r");
}
...
void DebugTask(void *argument)
{
    ...
    while (1)
    {
        second = GetSecond();
        xprintf("runtime: %ds\n\r", second);
        if ((pWriteBuffer = pvPortMalloc(500)) != NULL) {
...
void Thread_UartCmd(void *argument)     //to create a thread for scanf is infeasible because HAL_UART_Transmit() and HAL_UART_Receive() both use __HAL_LOCK
{
    while (1)
    {
