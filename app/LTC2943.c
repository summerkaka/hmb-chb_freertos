...
HAL_StatusTypeDef LTC2943_Write_mAh(stGauge *gauge,float level, osMutexId mutex)
{
    ...
    xprintf("LTC2943_Write_mAh(): write level: %f, wdata: %d, acr_ofuf: %d\n\r", level, wdata, gauge->acr_ofuf);

    ret = Write_LTC2943_Word(gauge,LTC2943_ACCUM_CHARGE_MSB_REG, wdata);
    osMutexRelease(mutex);
    return ret;
}
...
HAL_StatusTypeDef Get_Gauge_Information(stGauge *gauge, osMutexId mutex)
{
    ...
    ret = Read_LTC2943_Word(gauge, LTC2943_ACCUM_CHARGE_MSB_REG, &data_short);
    if (ret != HAL_OK)
        goto end;
    if (gauge->status & LTC2943_ACR_OVF_UDF) {
        xprintf("Get_Gauge_Information(): acr_OVF_UDF is detect, sts_reg is 0x%02x, acr reading is %d, origin level is %.0f\n\r", gauge->status, data_short, gauge->level);
        if (data_short > 0xffe0) {       // under flow
            gauge->acr_ofuf--;
    ...
    if (gauge->acr_ofuf < 0) {
        data_word = gauge->acr_ofuf * 65536l + (int32_t)data_short;
    } else if (gauge->acr_ofuf > 0) {
        data_word = gauge->acr_ofuf * 65535l + (int32_t)data_short;
    } else {
        data_word = data_short;
    }
    gauge->level = LTC2943_code_to_mAh(data_word, R_SENSE, PRESCALAR);

    //read voltage register
    ret = Read_LTC2943_Word(gauge,LTC2943_VOLTAGE_MSB_REG, &data_short);
    if (ret != HAL_OK)
        goto end;
    gauge->voltage = LTC2943_code_to_voltage(data_short);

    //read current register
    ret = Read_LTC2943_Word(gauge,LTC2943_CURRENT_MSB_REG, &data_short);
    if (ret != HAL_OK)
        goto end;
    gauge->current = (data_short == 0) ? 0 : LTC2943_code_to_current(data_short, R_SENSE);

    //read on-board temperature
    ret = Read_LTC2943_Word(gauge,LTC2943_TEMPERATURE_MSB_REG, &data_short);
    if (ret != HAL_OK)
        goto end;
    gauge->temperature = (data_short == 0) ? 0 : LTC2943_code_to_celcius_temperature(data_short);
