...
#define CHARGE_START_TH_TEMP    56      // can't start charge when temp > this
#define CHARGE_STOP_TH_TEMP     73      // force charge stop if temp > this
...
#define BAT_ERROR_TH_TEMP       90      // battery over-heat threshold
...
struct StBattery {
    ...
    float           volt_sample;                // sample bat volt every 1s to prevent spurious calibrate at reach-bottom when switch off
    float           last_voltage;               // to monitor voltage drop
    float           last_level;                 // for calculate remain time
    float           coulomb_per_prd;            // coulomb consumption per period
    ...
    uint32_t        tick_vpredict;              // tick to record battery vdrop, unit is second
    uint32_t        tick_lpredict;              // tick to record battery level-drop, unit is second
    uint32_t        tick_constcharge;           // tick to adjust const i_charge, and check const-charge-timer
    uint32_t        tick_volt_sample;
    ...
    uint32_t        cell_short_time;            // to avoid spurious error log for cell-short
    ...
    int32_t         chgtmr_update;              //
    uint16_t        charge_times;               // total charge times of this battery
    float           q_acm_discharge;            // total discharge coulomb
    float           q_start_charge;             // level at charge start
    float           q_estimate;                 // estimate level by efficiency
